import axios from "axios";
import { useCallback, useEffect, useState } from "react";
import { TbHandClick } from "react-icons/tb";
import { useParams } from "react-router-dom";
import Modal from "../components/Modal";
import RecordsForm from "../components/RecordsForm";
import { IoMdCopy } from "react-icons/io";
import toast, { Toaster } from "react-hot-toast";

const DnsRecords = () => {
  const parmas = useParams();
  const [loading, setLoading] = useState(true);
  const [recordsData, setRecordsData] = useState([]);
  const [recordValue, setRecordValue] = useState({});
  const [show, setShow] = useState(false);
  const getRecordsData = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `https://lucid-backend.onrender.com/api/domain/${parmas.hostedZoneId}`
      );
      setRecordsData(data.resourceRecordSets);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
    }
  }, [parmas.hostedZoneId]);
  useEffect(() => {
    if (!show) {
      getRecordsData();
    }
  }, [getRecordsData, show]);

  return (
    <div className="overflow-x-auto w-full flex flex-col gap-2 p-2 lg:px-5 py-3 h-full ">
      <h2 className="font-bold text-lg lg:text-3xl">Records Table</h2>
      {loading ? (
        <div className="w-full h-full flex justify-center items-center">
          <div role="status">
            <svg
              aria-hidden="true"
              className="w-8 h-8 text-gray-200 animate-spin  fill-purple-600"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                fill="currentColor"
              />
              <path
                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                fill="currentFill"
              />
            </svg>
            <span className="sr-only">Loading...</span>
          </div>
        </div>
      ) : (
        <>
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border border-gray-300">
            <thead className="text-xs text-gray-700 uppercase bg-gray-100 dark:text-gray-400">
              <tr className="border-b border-gray-300">
                <th scope="col" className="px-6 py-3 rounded-s-lg">
                  Name
                </th>
                <th scope="col" className="px-6 py-3 rounded-e-lg">
                  Type
                </th>
                <th scope="col" className="px-6 py-3 rounded-e-lg">
                  TTL
                </th>
                <th scope="col" className="px-6 py-3 rounded-e-lg">
                  Records
                </th>
                <th scope="col" className="px-6 py-3 rounded-e-lg">
                  Action
                </th>
              </tr>
            </thead>
            <tbody>
              {recordsData.map((item, index) => (
                <tr className="font-semibold text-gray-900 " key={index}>
                  <th scope="row" className="px-6 py-3 text-base">
                    {item.Name}
                  </th>
                  <td className="px-6 py-3">{item.Type}</td>
                  <td className="px-6 py-3">{item.TTL}</td>
                  <td className="px-6 py-3">
                    {item.ResourceRecords.map((item, index) => (
                      <p key={index} className="flex items-center">
                        {item.Value}{" "}
                        <span>
                          <IoMdCopy
                            onClick={() => {
                              navigator.clipboard.writeText(item.Value);
                              toast.success("copied");
                            }}
                          />
                        </span>
                      </p>
                    ))}
                  </td>
                  <td className="px-6 py-3">
                    <TbHandClick
                      className="cursor-pointer"
                      size={20}
                      onClick={() => {
                        setShow(true);
                        setRecordValue(item);
                      }}
                    />
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          <Modal show={show}>
            <RecordsForm
              recordValue={recordValue}
              hostedZoneId={parmas.hostedZoneId}
              handleCancel={() => setShow(false)}
            />
          </Modal>
          <Toaster />
        </>
      )}
    </div>
  );
};

export default DnsRecords;
